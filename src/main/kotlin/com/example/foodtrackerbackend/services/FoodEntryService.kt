...
class FoodEntryService(@Autowired private val mongoClient: MongoClient) {
    ...
    suspend fun saveNewFoodEntry(entryId: UUID, userId: String, entryTime: Timestamp, mealDescription: String, additionalComments: String, kilojoules: Int): UUID {
        //TODO save down to database
        val foodEntry = FoodEntry(entryId, userId, entryTime, mealDescription, additionalComments, kilojoules)
        val collection = getOrCreateCollection(database, "test")
        println(foodEntry)
